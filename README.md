...
* chanBlbChnRsec
  ...
  * Egress and ingress are separate agents with separate contexts (`chanBlbChnRsecEgrCtx` / `chanBlbChnRsecIgrCtx`); the application configures each direction independently. The size-of-wire fields (`dgramMax`, `tagSize`, `hmacSize`) must match between a sender's egress and the corresponding receiver's ingress. `chanBlbChnRsecShard()` returns the max shard size and `chanBlbChnRsecMax()` returns the max application payload for a given parity level m.
  ...
  * Statistics counters per direction (`chanBlbChnRsecEgrCtrs` / `chanBlbChnRsecIgrCtrs`) for production monitoring; coherent snapshots via `chanBlbChnRsecEgrSnap` / `chanBlbChnRsecIgrSnap`.
